...
var _ = Describe("Validations test", func() {
	...
	FContext("DNS validation", func() {
		...
		for _, domainType := range []struct {
			...
			successMessage := fmt.Sprintf("Domain name resolution for %s domain was successful or not required", domainType.domain)
			...
			pendingMessage := fmt.Sprintf("DNS validation for %s domain cannot be completed at the moment. This could be due to other validations", domainType.domain)
			...
			for _, dnsTestCase := range []struct {
				...
				It(fmt.Sprintf("%s - %s", domainType.destination, dnsTestCase.testCaseName), func() {
					var hostID, infraEnvID strfmt.UUID
